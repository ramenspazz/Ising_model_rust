extern crate rand;
use ising_model::atomistic_simulation::{SymmetryType, Driver};
use ising_model::*;
use ndarray::prelude::*;
...
fn main() {
    ...
    let mut parameters = sim_params::SimulationParameters::new(
        ...
        if usrin == 0 { SymmetryType::C3V } else { SymmetryType::C4V },
    ...
    fname = if usrin == 0 { "c3v_data.dat" } else { "c4v_data.dat" };
    ...
    loop {
        let user_run_stop = get_input_as_i64(Some(
            "Enter 0 to run a simulation, or -1 to quit: ",
        ));
        match user_run_stop.cmp(&0) {
            ...
            std::cmp::Ordering::Equal => {
                ...
                if first_run == true {
                    driver_obj.spin_energy(vec![100., 75., 50., 25., 10., 5., 1., 0.01], 1_000_000, 0, 1_000_000, true);
                    driver_obj.save_state(fname);
                    first_run = false;
                }
